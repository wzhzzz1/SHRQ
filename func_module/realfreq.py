...
def real_frequency_generation(dataset, data_size, domain_size, data_dimension, query_interval_table):
    user_histogram = np.zeros(domain_size, dtype=np.int32)  #生成数据主域大小的元素全为0的一维数组
    real_frequency_value = np.zeros(len(query_interval_table))  #用来保存真实的频率
    
    if data_dimension == 1:
        for k, item in enumerate(dataset):
            user_histogram[item] += 1                #用来统计元素的数量
    else:
        for k, item in enumerate(dataset):

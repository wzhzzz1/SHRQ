...
def ahead_tree_construction(ahead_tree, ahead_tree_height, theta, branch, translation_vector, user_dataset_partition):
    tree_height = 0
...
def ahead_tree_postprocessing(ahead_tree):
    ...
    for _, node in reversed(list(enumerate(ahead_tree.all_nodes()))):
        ...
        if ahead_tree.depth(node) != ahead_tree.depth() and ahead_tree.children(
...
def AHEAD_main_func(repeat_time, domain_size, branch, ahead_tree_height, theta, real_frequency, query_interval_table,
    ...
    while repeat < repeat_time:
        ...
        ahead_tree.create_node('Root', 'root',
        ...
        ahead_tree_construction(ahead_tree, ahead_tree_height, theta, branch, translation_vector,
                                user_dataset_partition)

        # ahead_tree post-processing
        ahead_tree_postprocessing(ahead_tree)

        # ahead_tree answer query
        ahead_tree_query_error_recorder(ahead_tree, real_frequency, query_interval_table, domain_size,
...
if __name__ == "__main__":
    ...
    # AHEAD-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    # 计算theta
    theta = theta_calculation(ahead_tree_height, epsilon, data_size, branch)  # 计算阈值theta
    AHEAD_main_func(repeat_time, domain_size, branch, ahead_tree_height, theta, real_frequency, query_interval_table,
                    epsilon, data_path, data_name, data_size_name, domain_name)

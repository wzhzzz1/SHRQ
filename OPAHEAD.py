...
def theta_calculation(ahead_tree_height, epsilon, user_scale, branch):
    ...
    varience_of_OUE = 4 * math.exp(epsilon) / (user_scale_in_each_layer * (math.exp(epsilon) - 1) ** 2) #每一层利用OUE所产生的误差
...
def theta1_calculation(ahead_tree_height, epsilon, user_scale, branch, d):
    ...
    #branch：分支数量，整数型，默认为2
    user_scale_in_each_layer = user_scale / ahead_tree_height    #利用用户的总数量除以ahead树的高度来计算每一层的用户数量
    varience_of_GRR =  (math.exp(epsilon)+d-2) / (user_scale_in_each_layer * (math.exp(epsilon) - 1) ** 2) #每一层利用OUE所产生的误差
...
def user_record_partition(data_path, ahead_tree_height, domain_size):
    ...
    user_sample_id = np.random.randint(0, ahead_tree_height, len(dataset)).reshape(len(dataset), 1)  # 先是生成从0到ahead树高度之间的随机整数，数量为数据集的大小的数组，然后把他们按照每列一个排列
    user_histogram = np.zeros((ahead_tree_height, domain_size), dtype=np.int32) #返回一个类型为int型，行数为ahead_tree_height，列数为domain_size大小的二维数组
...
def Optimized_Personalized_ahead_tree_update(ahead_tree, tree_height,  branch, translation_vector, layer_index, data_size,epsilon):
    num = len(ahead_tree.leaves())

    theta1 = theta1_calculation(ahead_tree_height, epsilon, data_size, branch,num) #计算阈值theta2
    theta2 = theta_calculation(ahead_tree_height, epsilon, data_size, branch) #计算阈值theta
    for node in ahead_tree.leaves():
        if not node.data.divide_flag:
            continue

        elif (tree_height > 0 and node.data.divide_flag) and (num < (3*math.exp(epsilon)+2) and node.data.frequency < theta1):
            node.data.divide_flag = False
            continue

        elif (tree_height > 0 and node.data.divide_flag) and (num >= (3*math.exp(epsilon)+2) and node.data.frequency < theta2):
            node.data.divide_flag = False
            continue
        else:
...
def Optimized_Personalized_ahead_tree_construction(ahead_tree, ahead_tree_height, branch, translation_vector, user_dataset_partition, data_size,epsilon_set):
    tree_height = 0
    while tree_height < ahead_tree_height:
        layer_index = 0
        # update ahead_tree structrue

        Optimized_Personalized_ahead_tree_update(ahead_tree, tree_height,  branch, translation_vector, layer_index, data_size,epsilon_set[tree_height])
        ...
        Optimized_node_frequency_aggregation(ahead_tree, user_dataset_partition[tree_height], epsilon_set[tree_height])
...
def ahead_tree_postprocessing(ahead_tree):
    ...
    for _, node in reversed(list(enumerate(ahead_tree.all_nodes()))):
        ...
        if lowest_nodes_number < ahead_tree.size(ahead_tree.depth()):  #.size()返回一层的所有节点  .depth()返回树的深度
            lowest_nodes_number += 1
            continue

        if ahead_tree.depth(node) != ahead_tree.depth() and ahead_tree.children(node.identifier) != []:  #判断时候是否为最后一层且是否存在孩子节点
...
def node_frequency_aggregation(ahead_tree, user_dataset, epsilon):
    ...
    noisy_frequency = freqoracle.Norm_Sub(noise_vector, len(noise_vector), sum(user_record_list), p, q) #返回经过Norm_Sub后处理过的频率
    ...
    for i, node in enumerate(ahead_tree.leaves()):
        if node.data.count == 0:
            ...
            node.data.frequency = ((node.data.count * node.data.frequency) + noisy_frequency[i]) / (node.data.count + 1)  #这一步它对于没有被划分的区间用多次频率取平均(原文好像没有)
            node.data.count += 1


def Optimized_node_frequency_aggregation(ahead_tree, user_dataset, epsilon):
    #ahead_tree:ahead树
    #user_dataset:高度为hight层的所有用户数据
    #epsilon：隐私预算
    # estimate the frequency values, and update the frequency values of the nodes
...
def Optimized_Personalized_AHEAD_main_func(repeat_time, domain_size, branch, ahead_tree_height, real_frequency, query_interval_table, epsilon_set, data_path, data_name, data_size_name, domain_name, data_size):
    MSEDict = {'rand': []}
    print("MSEDict",MSEDict)
    repeat = 0
    while repeat < repeat_time:
        # user partition
        user_dataset_partition = user_record_partition(data_path, ahead_tree_height, domain_size) #返回的是一个二维数组，每行包括用户的数据，列为树的高度

        # initialize the tree structure, set the root node
        ahead_tree = Tree() #初始化ahead树
        ahead_tree.create_node('Root', 'root', data=Nodex(1, True, 1, np.array([0, domain_size]))) #创建一个根节点为Root，存储的数据为Nodex的树

        # construct sub-domain partition vectors
        translation_vector = construct_translation_vector(domain_size, branch)

        # build a tree structure
        Optimized_Personalized_ahead_tree_construction(ahead_tree, ahead_tree_height, branch, translation_vector, user_dataset_partition, data_size,epsilon_set)

        # ahead_tree post-processing
        ahead_tree_postprocessing(ahead_tree)

        # ahead_tree answer query
        ahead_tree_query_error_recorder(ahead_tree, real_frequency, query_interval_table, domain_size, MSEDict) #将查询结果的均方误差返回字典里

        # record errors

        MSEDict_temp = pd.DataFrame.from_dict(MSEDict, orient='columns')  #pandas中的对数据处理工具

        MSEDict_temp.to_csv('rand_result/MSE_lle_Optimal_Personalized_ahead_branch{}-{}-{}-{}-{}-{}.csv'.format(branch,
                                                                                           data_name,
                                                                                           data_size_name,
                                                                                           domain_name,
                                                                                           epsilon,
...
if __name__ == "__main__":
    ...
    eps_set=np.linspace(epsilon,2,ahead_tree_height)
    print(eps_set)
    Optimized_Personalized_AHEAD_main_func(repeat_time, domain_size, branch, ahead_tree_height, real_frequency, query_interval_table, eps_set,data_path, data_name, data_size_name, domain_name, data_size)